...
class WPSEO_Breadcrumbs {
	...
	/**
	 * Filter: 'wpseo_breadcrumb_output_wrapper' - Allow changing the HTML wrapper element for the Yoast SEO
	 * breadcrumbs output
	 *
	...
	private function set_crumbs() {
		...
		if ( ( $this->show_on_front === 'page' && is_front_page() ) || ( $this->show_on_front === 'posts' && is_home() ) ) {
			...
			if ( is_post_type_archive() ) {
				...
				$this->add_predefined_crumb( $this->options[ 'breadcrumbs-archiveprefix' ] . ' ' . $user->display_name, null, true );
			} elseif ( is_search() ) {
				$this->add_predefined_crumb( $this->options[ 'breadcrumbs-searchprefix' ] . ' "' . esc_html( get_search_query() ) . '"', null, true );
			} elseif ( is_404() ) {
				...
				if ( 0 !== get_query_var( 'year' ) || ( 0 !== get_query_var( 'monthnum' ) || 0 !== get_query_var( 'day' ) ) ) {
					...
					$this->add_predefined_crumb( $this->options[ 'breadcrumbs-404crumb' ], null, true );
				}
			}
		}
	...
	private function add_home_crumb() {
		$this->add_predefined_crumb( $this->options[ 'breadcrumbs-home' ], get_home_url(), true );
	}
	...
	private function add_linked_month_year_crumb() {
		$this->add_predefined_crumb( $GLOBALS[ 'wp_locale' ]->get_month( get_query_var( 'monthnum' ) ) . ' ' . get_query_var( 'year' ), get_month_link( get_query_var( 'year' ), get_query_var( 'monthnum' ) ) );
	}
	...
	private function add_date_crumb( $date = null ) {
		...
		$this->add_predefined_crumb( $this->options[ 'breadcrumbs-archiveprefix' ] . ' ' . esc_html( $date ), null, true );
	}
	...
	private function add_year_crumb() {
		$this->add_predefined_crumb( $this->options[ 'breadcrumbs-archiveprefix' ] . ' ' . esc_html( get_query_var( 'year' ) ), null, true );
	}
	...
	/**
	 * Filter: 'wpseo_breadcrumb_output_class' - Allow changing the HTML class on the Yoast SEO breadcrumbs wrapper
	 * element
	 *
