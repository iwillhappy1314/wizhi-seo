...
class WPSEO_OpenGraph {
	...
	public function __construct() {
		...
		if ( isset( $GLOBALS[ 'fb_ver' ] ) || class_exists( 'Facebook_Loader', false ) ) {
			...
			add_filter( 'language_attributes', [ $this, 'add_opengraph_namespace' ] );
			...
			add_action( 'wpseo_opengraph', [ $this, 'site_owner' ], 20 );
	...
	public function add_opengraph_namespace( $input ) {
		return $input . ' prefix="og: http://ogp.me/ns#' . ( ( $this->options[ 'fbadminapp' ] != 0 || ( is_array( $this->options[ 'fb_admins' ] ) && $this->options[ 'fb_admins' ] !== [ ] ) ) ? ' fb: http://ogp.me/ns/fb#' : '' ) . '"';
	}
	...
	public function site_owner() {
		if ( isset( $this->options[ 'fbadminapp' ] ) && $this->options[ 'fbadminapp' ] != 0 ) {
			...
			return true;
		} else if ( isset( $this->options[ 'fb_admins' ] ) && is_array( $this->options[ 'fb_admins' ] ) && $this->options[ 'fb_admins' ] !== [ ] ) {
	...
	public function url() {
		 ...
		 */
		$url = apply_filters( 'wpseo_opengraph_url', WPSEO_Frontend::get_instance()
		                                                           ->canonical( false ) );
