...
class WPSEO_Frontend {
	...
	protected function __construct() {
		...
		add_action( 'wpseo_head', [ $this, 'debug_marker' ], 2 );
		add_action( 'wpseo_head', [ $this, 'robots' ], 6 );
		add_action( 'wpseo_head', [ $this, 'metadesc' ], 10 );
		add_action( 'wpseo_head', [ $this, 'metakeywords' ], 11 );
		add_action( 'wpseo_head', [ $this, 'canonical' ], 20 );
		add_action( 'wpseo_head', [ $this, 'adjacent_rel_links' ], 21 );
		add_action( 'wpseo_head', [ $this, 'publisher' ], 22 );
		...
		if ( ( $this->options[ 'disable-date' ] === true || $this->options[ 'disable-author' ] === true ) || ( isset( $this->options[ 'disable-post_formats' ] ) && $this->options[ 'disable-post_formats' ] ) ) {
		...
		if ( $this->options[ 'trailingslash' ] === true ) {
			add_filter( 'user_trailingslashit', [ $this, 'add_trailingslash' ], 10, 2 );
		}
	...
	private function generate_title( $title, $separator_location ) {
		...
		$separator = wpseo_replace_vars( '%%sep%%', [ ] );
	...
	public function robots() {
		...
		if ( is_singular() ) {
			...
			} elseif ( ( is_author() && $this->options[ 'noindex-author-wpseo' ] === true ) || ( is_date() && $this->options[ 'noindex-archive-wpseo' ] === true ) ) {
				$robots[ 'index' ] = 'noindex';
			} elseif ( is_home() ) {
	...
	public function metakeywords() {
		...
		if ( is_singular() ) {
			...
			if ( $this->is_home_posts_page() && $this->options[ 'metakey-home-wpseo' ] !== '' ) {
				$keywords = wpseo_replace_vars( $this->options[ 'metakey-home-wpseo' ], [ ] );
	...
	function archive_redirect() {
		...
		if ( ( $this->options[ 'disable-date' ] === true && $wp_query->is_date ) || ( $this->options[ 'disable-author' ] === true && $wp_query->is_author ) || ( isset( $this->options[ 'disable-post_formats' ] ) && $this->options[ 'disable-post_formats' ] && $wp_query->is_tax( 'post_format' ) ) ) {
	...
	public function clean_permalink() {
		 ...
		 */
		$whitelisted_extravars = apply_filters( 'wpseo_whitelist_permalink_vars', [ ] );

		if ( $this->options[ 'cleanpermalink-googlesitesearch' ] === true ) {
			// Prevent cleaning out Google Site searches.
			$whitelisted_extravars = array_merge( $whitelisted_extravars, [
				'q',
				'cx',
				'debug',
				'cof',
	...
	public function debug_marker( $echo = true ) {
		$marker = sprintf( '<!-- This site is optimized with the ' . $this->head_product_name() . '%1$s - https://yoast.com/wordpress/plugins/seo/ -->', /**
	...
	function title_test_helper( $title ) {
		...
		if ( $wpseo_titles[ 'title_test' ] > 5 ) {
			...
			remove_filter( 'wpseo_title', [ $this, 'title_test_helper' ] );
