...
class Yoast_Google_P12Signer extends Yoast_Google_Signer {
	...
	function __construct( $p12, $password ) {
		...
		if ( ! openssl_pkcs12_read( $p12, $certs, $password ) ) {
			throw new Yoast_Google_AuthException( "Unable to parse the p12 file.  " . "Is this a .p12 file?  Is the password correct?  OpenSSL error: " . openssl_error_string() );
		}
	...
	function sign( $data ) {
		if ( version_compare( PHP_VERSION, '5.3.0' ) < 0 ) {
			throw new Yoast_Google_AuthException( "PHP 5.3.0 or higher is required to use service accounts." );
		}
