...
const MAX_TOKEN_LIFETIME_SECS = 86400;
	...
	public function authenticate( $service, $code = null ) {
		...
		if ( $code ) {
			...
			$request = Yoast_Google_Client::$io->makeRequest( new Yoast_Google_HttpRequest( self::OAUTH2_TOKEN_URI, 'POST', [ ], [
				'code'          => $code,
	...
	public function createAuthUrl( $scope ) {
		...
		if ( strpos( $scope, 'plus.login' ) && count( $this->requestVisibleActions ) > 0 ) {
			$params[] = 'request_visible_actions=' . urlencode( $this->requestVisibleActions );
		}
		...
		if ( $this->isAccessTokenExpired() ) {
			if ( $this->assertionCredentials ) {
				...
				if ( ! array_key_exists( 'refresh_token', $this->token ) ) {
					throw new Yoast_Google_AuthException( "The OAuth 2.0 access token has expired, " . "and a refresh token is not available. Refresh tokens are not " . "returned for responses that were auto-approved." );
				}
	...
	private function refreshTokenRequest( $params ) {
		$http    = new Yoast_Google_HttpRequest( self::OAUTH2_TOKEN_URI, 'POST', [ ], $params );
	...
	public function revokeToken( $token = null ) {
		...
		$request  = new Yoast_Google_HttpRequest( self::OAUTH2_REVOKE_URI, 'POST', [ ], "token=$token" );
	...
	private function getFederatedSignOnCerts() {
		...
		$request = Yoast_Google_Client::$io->makeRequest( new Yoast_Google_HttpRequest( self::OAUTH2_FEDERATED_SIGNON_CERTS_URL ) );
		if ( $request->getResponseHttpCode() == 200 ) {
		...
		throw new Yoast_Google_AuthException( "Failed to retrieve verification certificates: '" . $request->getResponseBody() . "'.", $request->getResponseHttpCode() );
	}
