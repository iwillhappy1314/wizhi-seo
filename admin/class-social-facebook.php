...
class Yoast_Social_Facebook {
	...
	private function cleanup_referrer_url() {
		$_SERVER[ 'REQUEST_URI' ] = remove_query_arg( func_get_args(), filter_input( INPUT_SERVER, 'REQUEST_URI', FILTER_CALLBACK, [ 'options' => 'sanitize_text_field' ] ) );
	}
...
class Yoast_Social_Facebook_Form {
	...
	private function manage_app_as_admin() {
		...
		Yoast_Form::get_instance()
		          ->textinput( 'fbadminapp', __( 'Facebook App ID', 'wordpress-seo' ) );
	...
	/**
	 * Display the buttons to add an admin or add another admin from Facebook and display the admin that has been added
	 * already.
	 *
	 * @return $this
	 */
	...
	private function form_head() {
		...
		echo sprintf( esc_html__( 'To be able to access %sFacebook Insights%s for your site, you need to specify a Facebook Admin. This can be a user. If you have an app for your site, you could use that as well.', 'wordpress-seo' ), '<a target="_blank" href="https://www.facebook.com/insights">', '</a>' );
		echo ' ';
		/* translators: %1$s and %2$s expand to a link to the Yoast Knowledge Base */
