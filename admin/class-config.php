...
class WPSEO_Admin_Pages {
	...
	function config_page_styles() {
		...
		wp_enqueue_style( 'yoast-admin-css', plugins_url( 'css/yst_plugin_tools' . WPSEO_CSSJS_SUFFIX . '.css', WPSEO_FILE ), [ ], WPSEO_VERSION );

		if ( is_rtl() ) {
			wp_enqueue_style( 'wpseo-rtl', plugins_url( 'css/wpseo-rtl' . WPSEO_CSSJS_SUFFIX . '.css', WPSEO_FILE ), [ ], WPSEO_VERSION );
		}
	}
	...
	function config_page_scripts() {
		wp_enqueue_script( 'wpseo-admin-script', plugins_url( 'js/wp-seo-admin' . WPSEO_CSSJS_SUFFIX . '.js', WPSEO_FILE ), [
			'jquery',
			'jquery-ui-core',
		], WPSEO_VERSION, true );
		...
		if ( in_array( $page, [ 'wpseo_social', 'wpseo_dashboard' ] ) ) {
			wp_enqueue_media();
		...
		if ( 'wpseo_tools' === $page && 'bulk-editor' === $tool ) {
			wp_enqueue_script( 'wpseo-bulk-editor', plugins_url( 'js/wp-seo-bulk-editor' . WPSEO_CSSJS_SUFFIX . '.js', WPSEO_FILE ), [ 'jquery' ], WPSEO_VERSION, true );
		}

		if ( 'wpseo_tools' === $page && 'import-export' === $tool ) {
			wp_enqueue_script( 'wpseo-export', plugins_url( 'js/wp-seo-export' . WPSEO_CSSJS_SUFFIX . '.js', WPSEO_FILE ), [ 'jquery' ], WPSEO_VERSION, true );
		}
	}
	...
	public function admin_header( $form = true, $option_long_name = false, $option = 'wpseo', $contains_files = false ) {
		...
		Yoast_Form::get_instance()
		          ->admin_header( $form, $option, $contains_files, $option_long_name );
	}
	...
	public function admin_footer( $submit = true, $show_sidebar = true ) {
		...
		Yoast_Form::get_instance()
		          ->admin_footer( $submit, $show_sidebar );
	}
	 ...
	 */
	function textarea( $var, $label, $option = '', $attr = [ ] ) {
		_deprecated_function( __METHOD__, 'WPSEO 2.0', 'This method is deprecated, please use the <code>Yoast_Form</code> class.' );
