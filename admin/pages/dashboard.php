...
if ( isset( $_GET[ 'fixmetadesc' ] ) && check_admin_referer( 'wpseo-fix-metadesc', 'nonce' ) && $options[ 'theme_description_found' ] !== '' ) {
	...
	if ( isset( $_SERVER[ 'REQUEST_URI' ] ) ) {
		$_SERVER[ 'REQUEST_URI' ] = remove_query_arg( [ 'nonce', 'fixmetadesc' ], $_SERVER[ 'REQUEST_URI' ] );
	}
}
...
if ( isset( $_GET[ 'checkmetadesc' ] ) ) {
	// Clean up the referrer url for later use.
	if ( isset( $_SERVER[ 'REQUEST_URI' ] ) ) {
		$_SERVER[ 'REQUEST_URI' ] = remove_query_arg( [ 'nonce', 'checkmetadesc' ], $_SERVER[ 'REQUEST_URI' ] );
	}
}
...
if ( $options[ 'theme_description_found' ] !== '' ) {
	...
	echo '<a href="', esc_url( add_query_arg( [ 'nonce' => wp_create_nonce( 'wpseo-fix-metadesc' ) ], admin_url( 'admin.php?page=wpseo_dashboard&fixmetadesc' ) ) ), '" class="button fixit">', __( 'Fix it.', 'wordpress-seo' ), '</a>';
	echo ' <a href="', esc_url( add_query_arg( [ 'nonce' => wp_create_nonce( 'wpseo-check-metadesc' ) ], admin_url( 'admin.php?page=wpseo_dashboard&checkmetadesc' ) ) ), '" class="button checkit">', __( 'Re-check theme.', 'wordpress-seo' ), '</a>';
	/* translators: %1$s expands to Yoast SEO */
	...
	<div id="general" class="wpseotab">
		...
		<p>
			<a onclick="if( !confirm('<?php _e( 'Are you sure you want to reset your SEO settings?', 'wordpress-seo' ); ?>') ) return false;"
			   class="button"
			   href="<?php echo esc_url( add_query_arg( [ 'nonce' => wp_create_nonce( 'wpseo_reset_defaults' ) ], admin_url( 'admin.php?page=wpseo_dashboard&wpseo_reset_defaults=1' ) ) ); ?>"><?php _e( 'Restore Default Settings', 'wordpress-seo' ); ?></a>
		</p>
	</div>
