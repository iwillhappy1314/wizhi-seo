...
if ( isset( $_POST[ 'wpseo_submit' ] ) ) {
	...
	foreach ( [ 'access', 'defaultblog' ] as $opt ) {
...
$yform->select( 'access', /* translators: %1$s expands to Yoast SEO */
...
if ( $use_dropdown === true ) {
	$yform->select( 'defaultblog', __( 'New sites in the network inherit their SEO settings from this site', 'wordpress-seo' ), $dropdown_input, 'wpseo_ms' );
...
if ( $use_dropdown === true ) {
	...
	$yform->select( 'restoreblog', __( 'Site ID', 'wordpress-seo' ), $dropdown_input, 'wpseo_ms' );
} else {
