...
class WPSEO_Admin_Init {
	...
	public function __construct() {
		...
		add_action( 'admin_init', [ $this, 'after_update_notice' ], 15 );
	...
	private function load_admin_user_class() {
		if ( in_array( $this->pagenow, [ 'user-edit.php', 'profile.php' ] ) && current_user_can( 'edit_users' ) ) {
	...
	private function load_tour() {
		...
		if ( ! get_user_meta( get_current_user_id(), 'wpseo_ignore_tour' ) ) {
			add_action( 'admin_enqueue_scripts', [ 'WPSEO_Pointers', 'get_instance' ] );
		}
	}
	...
	public function enqueue_dismissible() {
		if ( version_compare( $GLOBALS[ 'wp_version' ], '4.2', '<' ) ) {
			wp_enqueue_style( 'wpseo-dismissible', plugins_url( 'css/wpseo-dismissible' . WPSEO_CSSJS_SUFFIX . '.css', WPSEO_FILE ), [ ], WPSEO_VERSION );
			wp_enqueue_script( 'wpseo-dismissible', plugins_url( 'js/wp-seo-dismissible' . WPSEO_CSSJS_SUFFIX . '.js', WPSEO_FILE ), [ 'jquery' ], WPSEO_VERSION, true );
		}
	}
	...
	public function after_update_notice() {
		if ( current_user_can( 'manage_options' ) && ! $this->seen_about() ) {
			...
			$info_message = sprintf( __( '%1$s has been updated to version %2$s. %3$sClick here%4$s to find out what\'s new!', 'wordpress-seo' ), 'Yoast SEO', WPSEO_VERSION, '<a href="' . admin_url( 'admin.php?page=wpseo_dashboard&intro=1' ) . '">', '</a>' );
			...
			Yoast_Notification_Center::get()
			                         ->add_notification( new Yoast_Notification( $info_message, $notification_options ) );
		}
	}
	...
	public function ga_compatibility_notice() {
		if ( defined( 'GAWP_VERSION' ) && '5.4.3' === GAWP_VERSION ) {
			...
			$info_message = sprintf( /* translators: %1$s expands to Yoast SEO, %2$s expands to 5.4.3, %3$s expands to Google Analytics by Yoast */
				__( '%1$s detected you are using version %2$s of %3$s, please update to the latest version to prevent compatibility issues.', 'wordpress-seo' ), 'Yoast SEO', '5.4.3', 'Google Analytics by Yoast' );
