...
class WPSEO_Sitemaps_Admin {
	...
	function __construct() {
		...
		add_action( 'admin_init', [ $this, 'delete_sitemaps' ] );
	}
	...
	function delete_sitemaps() {
		...
		if ( $options[ 'enablexmlsitemap' ] === true ) {
			...
			foreach ( $file_to_check_for as $file ) {
				if ( ( $options[ 'blocking_files' ] === [ ] || ( $options[ 'blocking_files' ] !== [ ] && in_array( $file, $options[ 'blocking_files' ] ) === false ) ) && file_exists( $file ) ) {
