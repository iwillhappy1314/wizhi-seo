...
class WPSEO_Bulk_List_Table extends WP_List_Table {
	...
	private function verify_nonce() {
		if ( $this->should_verify_nonce() && ! wp_verify_nonce( filter_input( INPUT_GET, 'nonce' ), 'bulk-editor-table' ) ) {
			Yoast_Notification_Center::get()
			                         ->add_notification( new Yoast_Notification( __( 'You are not allowed to access this page.', 'wordpress-seo' ), [ 'type' => 'error' ] ) );
			Yoast_Notification_Center::get()
	...
	protected function populate_editable_post_types() {
		$post_types = get_post_types( [ 'public' => true, 'exclude_from_search' => false ], 'object' );
	...
	function prepare_page_navigation() {
		...
		if ( $_GET[ 'type' ] != $this->page_type ) {
			...
			$current_filter = '-1';
			$current_status = '';
			$current_order  = [ 'orderby' => 'post_title', 'order' => 'asc' ];
	 ...
	 *
	 * @return string $subquery The subquery, which should always be used in $wpdb->prepare(), passing the current
	 *                user_id in as the first parameter.
	 */
	...
	/**
	 * Heavily restricts the possible columns by which a user can order the table in the bulk editor, thereby
	 * preventing a possible CSRF vulnerability.
	 *
	...
	/**
	 * Makes sure the order clause is always ASC or DESC for the bulk editor table, thereby preventing a possible CSRF
	 * vulnerability.
	 *
	...
	protected function get_items( $query ) {
		...
		$this->items = $wpdb->get_results( $wpdb->prepare( $query, $this->pagination[ 'offset' ], $this->pagination[ 'per_page' ] ) );
	}
	...
	function get_views() {
		...
		if ( is_array( $post_stati ) && $post_stati !== [ ] ) {
			foreach ( $post_stati as $status ) {
				...
				$status_links[ $status_name ] = '<a href="' . esc_url( add_query_arg( [ 'post_status' => $status_name ], admin_url( 'admin.php?page=wpseo_tools&tool=bulk-editor' . $this->page_url ) ) ) . '"' . $class . '>' . sprintf( translate_nooped_plural( $status->label_count, $total ), number_format_i18n( $total ) ) . '</a>';
			}
		}
	...
	protected function parse_page_title_column( $rec ) {
		...
		if ( $post_type_object->public ) {
			if ( in_array( $rec->post_status, [ 'pending', 'draft', 'future' ] ) ) {
				if ( $can_edit_post ) {
