...
class WPSEO_Taxonomy {
	...
	function __construct() {
		...
		if ( is_admin() && ( isset( $_GET[ 'taxonomy' ] ) && $_GET[ 'taxonomy' ] !== '' ) && ( ! isset( $options[ 'hideeditbox-tax-' . $_GET[ 'taxonomy' ] ] ) || $options[ 'hideeditbox-tax-' . $_GET[ 'taxonomy' ] ] === false ) ) {
			add_action( sanitize_text_field( $_GET[ 'taxonomy' ] ) . '_edit_form', [
				$this,
				'term_seo_form',
			], 90, 1 );
		}

		add_action( 'split_shared_term', [ $this, 'split_shared_term' ], 10, 4 );
		...
		add_action( 'admin_init', [ $this, 'translate_meta_options' ] );
	 ...
	 *
	 * @link https://make.wordpress.org/core/2015/02/16/taxonomy-term-splitting-in-4-2-a-developer-guide/ Article
	 *       explaining the taxonomy term splitting in WP 4.2.
	 *
	...
	function admin_enqueue_scripts() {
		if ( $GLOBALS[ 'pagenow' ] === 'edit-tags.php' && ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] === 'edit' ) ) {
			wp_enqueue_style( 'yoast-taxonomy-css', plugins_url( 'css/taxonomy-meta' . WPSEO_CSSJS_SUFFIX . '.css', WPSEO_FILE ), [ ], WPSEO_VERSION );
		}
	}
	 ...
	 */
	function form_row( $var, $label, $desc, $tax_meta, $type = 'text', $options = [ ] ) {
		$val = '';
