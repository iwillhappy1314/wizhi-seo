...
class WPSEO_Admin {
	...
	function __construct() {
		...
		if ( '0' == get_option( 'blog_public' ) ) {
			add_action( 'admin_footer', [ $this, 'blog_public_warning' ] );
		}
		...
		if ( ( ( isset( $this->options[ 'theme_has_description' ] ) && $this->options[ 'theme_has_description' ] === true ) || $this->options[ 'theme_description_found' ] !== '' ) && $this->options[ 'ignore_meta_description_warning' ] !== true ) {
			add_action( 'admin_footer', [ $this, 'meta_description_warning' ] );
		}

		if ( $this->options[ 'cleanslugs' ] === true ) {
			add_filter( 'name_save_pre', [ $this, 'remove_stopwords_from_slug' ], 0 );
		}
	...
	function register_settings_page() {
		...
		// Add main page.
		$admin_page = add_menu_page( 'Yoast SEO: ' . __( 'General Settings', 'wordpress-seo' ), __( 'SEO', 'wordpress-seo' ), 'manage_options', 'wpseo_dashboard', [
			$this,
			'load_page',
		], 'dashicons-analytics', '99.31337' );
		...
		$submenu_pages = [
			...
			[
				...
				[ $this->page_gsc, 'display' ],
				[ [ $this->page_gsc, 'set_help' ] ],
			],
		];
		...
		if ( count( $submenu_pages ) ) {
			foreach ( $submenu_pages as $submenu_page ) {
				...
				if ( isset( $submenu_page[ 6 ] ) && ( is_array( $submenu_page[ 6 ] ) && $submenu_page[ 6 ] !== [ ] ) ) {
	...
	function register_network_settings_page() {
		if ( WPSEO_Utils::grant_access() ) {
			...
			add_menu_page( 'Yoast SEO: ' . __( 'MultiSite Settings', 'wordpress-seo' ), __( 'SEO', 'wordpress-seo' ), 'delete_users', 'wpseo_dashboard', [
				$this,
				'network_config_page',
			], $icon_svg );

			if ( WPSEO_Utils::allow_system_file_edit() === true ) {
				add_submenu_page( 'wpseo_dashboard', 'Yoast SEO: ' . __( 'Edit Files', 'wordpress-seo' ), __( 'Edit Files', 'wordpress-seo' ), 'delete_users', 'wpseo_files', [
					$this,
					'load_page',
				] );
			}
	...
	function blog_public_warning() {
		...
		printf( '
			...
			</div>', __( 'Huge SEO Issue: You\'re blocking access to robots.', 'wordpress-seo' ), sprintf( __( 'You must %sgo to your Reading Settings%s and uncheck the box for Search Engine Visibility.', 'wordpress-seo' ), sprintf( '<a href="%s">', esc_url( admin_url( 'options-reading.php' ) ) ), '</a>' ), esc_js( wp_create_nonce( 'wpseo-ignore' ) ), __( 'I know, don\'t bug me.', 'wordpress-seo' ) );
	}
	...
	function meta_description_warning() {
		...
		printf( '
			...
			</div>', __( 'SEO Issue:', 'wordpress-seo' ), /* translators: %1$s expands to Yoast SEO, %2$s to opening anchor and %3$s the anchor closing tag */
			sprintf( __( 'Your theme contains a meta description, which blocks %1$s from working properly. Please visit the %2$sSEO Dashboard%3$s to fix this.', 'wordpress-seo' ), 'Yoast SEO', sprintf( '<a href="%s">', esc_url( admin_url( 'admin.php?page=wpseo_dashboard' ) ) ), '</a>' ), esc_js( wp_create_nonce( 'wpseo-ignore' ) ), __( 'I know, don\'t bug me.', 'wordpress-seo' ) );
	}
	...
	function config_page_scripts() {
		if ( WPSEO_Utils::grant_access() ) {
			wp_enqueue_script( 'wpseo-admin-global-script', plugins_url( 'js/wp-seo-admin-global' . WPSEO_CSSJS_SUFFIX . '.js', WPSEO_FILE ), [ 'jquery' ], WPSEO_VERSION, true );
		}
	}
