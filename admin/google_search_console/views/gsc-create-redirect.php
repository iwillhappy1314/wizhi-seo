			...
			switch ( $view_type ) {
				case 'create' :
					...
					<div class='form-field form-required'>
						...
						<input type='text' id='wpseo-current-url' name='current_url' value='<?php echo $url; ?>'
						       disabled='disabled'>
					</div>
					<div class='form-field form-required'>
					...
					<div class='form-field form-required'>
						<label for='wpseo-mark-as-fixed'
						       class='clear'><?php _e( 'Mark as fixed:', 'wordpress-seo' ); ?></label>
						<input type='checkbox' checked value='1' id='wpseo-mark-as-fixed' name='mark_as_fixed'
						       class='clear'>
						<p><?php
					...
					<p class='submit'>
						<input type='button' name='submit' id='submit' class='button button-primary'
						       value='<?php _e( 'Create redirect', 'wordpress-seo' ); ?>'
				...
				case 'already_exists' :
					...
					echo sprintf( __( 'You do not have to create a redirect for URL %1$s because a redirect already exists. The existing redirect points to %2$s. If this is fine you can mark this issue as fixed. If not, please go to the redirects page and change the target URL.', 'wordpress-seo' ), $url, $current_redirect );
					echo '</p>';
					break;
				...
				case 'no_premium' :
					...
					echo sprintf( __( 'To be able to create a redirect and fix this issue, you need %1$s. You can buy the plugin, including one year support and updates, on %2$s.', 'wordpress-seo' ), 'Yoast SEO Premium', '<a href="http://yoa.st/redirects" target="_blank">yoast.com</a>' );
					echo '</p>';
					break;
			}
			?>
