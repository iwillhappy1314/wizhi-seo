...
class WPSEO_GSC_Service {
	...
	private function set_client() {
		try {
			...
			if ( $exception->getMessage() === 'required_version' ) {
				$this->incompatible_api_libs( __( 'Yoast plugins share some code between them to make your site faster. As a result of that, we need all Yoast plugins to be up to date. We\'ve detected this isn\'t the case, so please update the Yoast plugins that aren\'t up to date yet.', 'wordpress-seo' ) );
			}
		}

		if ( class_exists( 'Yoast_Api_Google_Client' ) === false ) {
			$this->incompatible_api_libs( /* translators: %1$s expands to Yoast SEO, %2$s expands to Google Analytics by Yoast */
				sprintf( __( '%1$s detected you’re using a version of %2$s which is not compatible with %1$s. Please update %2$s to the latest version to use this feature.', 'wordpress-seo' ), 'Yoast SEO', 'Google Analytics by Yoast' ) );
	...
	private function incompatible_api_libs( $notice ) {
		Yoast_Notification_Center::get()
		                         ->add_notification( new Yoast_Notification( $notice, [ 'type' => 'error' ] ) );
	}
	...
	public function fetch_category_issues( $platform, $category ) {
		$issues = $this->client->do_request( 'sites/' . urlencode( $this->profile ) . '/urlCrawlErrorsSamples?category=' . $category . '&platform=' . $platform, true );
