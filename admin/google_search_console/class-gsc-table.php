...
class WPSEO_GSC_Table extends WP_List_Table {
	...
	protected function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="wpseo_crawl_issues[]" value="%s" />', $item[ 'url' ] );
	}
	...
	protected function column_url( $item ) {
		...
		return sprintf( '<span class="value">%1$s</span> %2$s', $item[ 'url' ], $this->row_actions( $actions ) );
	}
	...
	private function can_create_redirect() {
		return in_array( $this->current_view, [ 'soft_404', 'not_found', 'access_denied' ] );
	}
