...
class WPSEO_GSC {
	...
	private function add_notification( $message, $type ) {
		Yoast_Notification_Center::get()
		                         ->add_notification( new Yoast_Notification( $message, [ 'type' => $type ] ) );
	}

	/**
	 * If the Google Search Console has no credentials, add a notification for the user to give him a heads up. This
	 * message is dismissable.
	 */
	public function register_gsc_notification() {
		Yoast_Notification_Center::get()
		                         ->add_notification( new Yoast_Notification( sprintf( __( 'Don\'t miss your crawl errors: %1$sconnect with Google Search Console here%2$s.', 'wordpress-seo' ), '<a href="' . admin_url( 'admin.php?page=wpseo_search_console&tab=settings' ) . '">', '</a>' ), [
	...
	public function page_scripts() {
		wp_enqueue_script( 'wp-seo-admin-gsc', plugin_dir_url( WPSEO_FILE ) . 'js/wp-seo-admin-gsc' . WPSEO_CSSJS_SUFFIX . '.js', [ 'jquery' ], WPSEO_VERSION );
		...
		wp_enqueue_style( 'jquery-qtip.js', plugins_url( 'css/jquery.qtip' . WPSEO_CSSJS_SUFFIX . '.css', WPSEO_FILE ), [ ], WPSEO_VERSION );
		wp_enqueue_style( 'metabox-tabs', plugins_url( 'css/metabox-tabs' . WPSEO_CSSJS_SUFFIX . '.css', WPSEO_FILE ), [ ], WPSEO_VERSION );
		wp_enqueue_script( 'jquery-qtip', plugins_url( 'js/jquery.qtip.min.js', WPSEO_FILE ), [ 'jquery' ], WPSEO_VERSION, true );
	}
