...
class WPSEO_GSC_Category_Filters {
	...
	private function create_view_link( $category, $count ) {
		$href = add_query_arg( [ 'category' => $category, 'paged' => 1 ] );
		...
		return sprintf( '<a href="%1$s" class="%2$s" %3$s>%4$s</a> (<span id="gsc_count_%5$s">%6$s</span>)', esc_attr( $href ), $class, $title, $this->filter_values[ $category ][ 'value' ], $category, $count );
	}
