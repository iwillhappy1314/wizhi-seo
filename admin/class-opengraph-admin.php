...
class WPSEO_Social_Admin extends WPSEO_Metabox {
	...
	public function __construct() {
		...
		add_action( 'wpseo_tab_header', [ $this, 'tab_header' ], 60 );
		add_action( 'wpseo_tab_content', [ $this, 'tab_content' ] );
	...
	public function og_data_compare( $post ) {
		...
		if ( ! empty( $_POST ) && ! empty( $post->ID ) && $post->post_status == 'publish' && isset( $_POST[ 'original_post_status' ] ) && $_POST[ 'original_post_status' ] === 'publish' ) {
			...
			if ( $reset_facebook_cache ) {
				wp_remote_get( 'https://graph.facebook.com/?id=' . get_permalink( $post->ID ) . '&scrape=true&method=post' );
			}
		}
	}
