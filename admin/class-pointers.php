...
class WPSEO_Pointers {
	...
	private function __construct() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( ! get_user_meta( get_current_user_id(), 'wpseo_ignore_tour' ) ) {
				...
				add_action( 'admin_print_footer_scripts', [ $this, 'intro_tour' ] );
			}
		}
	}
	...
	private function do_page_pointer( $page ) {
		...
		$pointer = call_user_func( [ $this, $this->admin_pages[ $page ] ] );
	...
	private function start_tour_pointer() {
		...
		$content  = '<h3>' . __( 'Congratulations!', 'wordpress-seo' ) . '</h3>' /* translators: %1$s expands to Yoast SEO */ . '<p>' . sprintf( __( 'You&#8217;ve just installed %1$s! Click &#8220;Start Tour&#8221; to view a quick introduction of this plugin&#8217;s core functionality.', 'wordpress-seo' ), 'Yoast SEO' ) . '</p>';
		$opt_arr  = [
			'content'  => $content,
	...
	private function dashboard_pointer() {
		...
		return [
			'content'   => '<h3>' . __( 'General settings', 'wordpress-seo' ) . '</h3>' /* translators: %1$s expands to Yoast SEO */ . '<p>' . sprintf( __( 'These are the General settings for %1$s, here you can restart this tour or revert the %1$s settings to default.', 'wordpress-seo' ), 'Yoast SEO' ) . '</p>' . '<p><strong>' . __( 'Tab: Your Info / Company Info', 'wordpress-seo' ) . '</strong><br/>' . __( 'Add some info here needed for Google\'s Knowledge Graph.', 'wordpress-seo' ) . '</p>' . '<p><strong>' . __( 'Tab: Webmaster Tools', 'wordpress-seo' ) . '</strong><br/>' . __( 'You can add the verification codes for the different Webmaster Tools programs here. We highly encourage you to check out both Google and Bing&#8217;s Webmaster Tools.', 'wordpress-seo' ) . '</p>' . '<p><strong>' . __( 'Tab: Security', 'wordpress-seo' ) . '</strong><br/>' . __( 'Determine who has access to the plugins advanced settings on the post edit screen.', 'wordpress-seo' ) . '</p>'

			               /* translators: %1$s expands to Yoast SEO */ . '<p><strong>' . sprintf( __( 'More %1$s', 'wordpress-seo' ), 'Yoast SEO' ) . '</strong><br/>'

			               /* @todo What about this translation */ . sprintf( __( 'There&#8217;s more to learn about WordPress &amp; SEO than just using this plugin. A great start is our article %1$sthe definitive guide to WordPress SEO%2$s.', 'wordpress-seo' ), '<a target="_blank" href="' . esc_url( 'https://yoast.com/articles/wordpress-seo/#utm_source=wpseo_dashboard&utm_medium=wpseo_tour&utm_campaign=tour' ) . '">', '</a>' ) . '</p>' . '<p><strong style="font-size:150%;">' . __( 'Subscribe to our Newsletter', 'wordpress-seo' ) . '</strong><br/>' /* translators: %1$s expands to Yoast SEO */ . sprintf( __( 'If you would like us to keep you up-to-date regarding %1$s and other plugins by Yoast, subscribe to our newsletter:', 'wordpress-seo' ), 'Yoast SEO' ) . '</p>' . '<form target="_blank" action="http://yoast.us1.list-manage1.com/subscribe/post?u=ffa93edfe21752c921f860358&amp;id=972f1c9122" method="post" selector="newsletter-form" accept-charset="' . esc_attr( get_bloginfo( 'charset' ) ) . '">' . '<p>' . '<input style="margin: 5px; color:#666" name="EMAIL" value="' . esc_attr( $current_user->user_email ) . '" selector="newsletter-email" placeholder="' . __( 'Email', 'wordpress-seo' ) . '"/>' . '<input type="hidden" name="group" value="2"/>' . '<button type="submit" class="button-primary">' . __( 'Subscribe', 'wordpress-seo' ) . '</button>' . '</p>' . '</form>',
			'next_page' => 'titles',
		];
	}
