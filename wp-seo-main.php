...
function wpseo_network_activate_deactivate( $activate = true ) {
	...
	if ( is_array( $all_blogs ) && $all_blogs !== [ ] ) {
...
function wpseo_frontend_head_init() {
	...
	if ( $options[ 'googleplus' ] === true && is_singular() ) {
		add_action( 'wpseo_head', [ 'WPSEO_GooglePlus', 'get_instance' ], 35 );
	}
}
...
register_activation_hook( WPSEO_FILE, [ 'WPSEO_Plugin_Conflict', 'hook_check_for_plugin_conflicts' ] );
