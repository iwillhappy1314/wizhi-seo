...
abstract class WPSEO_Option {
	...
	protected function __construct() {
		...
		if ( $this->multisite_only !== true ) {
			...
			if ( version_compare( $GLOBALS[ 'wp_version' ], '3.7', '!=' ) ) { // Adding back after non-WP 3.7 UPDATE.
				...
				add_filter( 'pre_update_option_' . $this->option_name, [ $this, 'wp37_add_default_filters' ] );
			}
			...
			if ( $meta !== '' ) {
				...
				if ( preg_match( $regex, $meta ) ) {
					...
					if ( function_exists( 'add_settings_error' ) ) {
						add_settings_error( $this->group_name, // Slug title of the setting.
	 ...
	 *
	 * @param  array $options (Optional) Current options. If not set, the option defaults for the $option_key will be
	 *                        returned.
	 *
	 ...
	 *
	 * @param  string $current_version (optional) Version from which to upgrade, if not set, version specific upgrades
	 *                                 will be disregarded.
	 *
	 * @return void
	 */
	 ...
	 * @param  array  $all_old_option_values (optional) Only used when importing old options to have access to the real
	 *                                       old values, in contrast to the saved ones.
	 *
	 * @return void
	 */
