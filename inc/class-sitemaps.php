...
class WPSEO_Sitemaps {
	...
	function __construct() {
		...
		add_filter( 'wpseo_sitemap_exclude_author', [ $this, 'user_sitemap_remove_excluded_authors' ], 8 );
	...
	function build_root_map() {
		...
		if ( is_array( $taxonomy_names ) && $taxonomy_names !== [ ] ) {
			foreach ( $taxonomy_names as $tax ) {
				if ( in_array( $tax, [ 'link_category', 'nav_menu', 'post_format' ] ) ) {
		...
		if ( $this->options[ 'disable-author' ] === false && $this->options[ 'disable_author_sitemap' ] === false ) {
			...
			for ( $i = 0; $i < $n; $i ++ ) {
				...
				if ( empty( $count ) || $count == $n ) {
					$date = $wpdb->get_var( $wpdb->prepare( $date_query . ' ASC LIMIT 1', $wpdb->get_blog_prefix() . 'user_level' ) );

					// Retrieve the newest updated profile timestamp by an offset.
				} else {
					$date = $wpdb->get_var( $wpdb->prepare( $date_query . ' DESC LIMIT 1 OFFSET %d', $wpdb->get_blog_prefix() . 'user_level', ( ( $this->max_entries * ( $i + 1 ) ) - 1 ) ) );
				}
	...
	function get_last_modified( $post_types ) {
		...
		if ( ! is_array( $this->post_type_dates ) ) {
			$this->post_type_dates = [ ];
			$query                 = "SELECT post_type, MAX(post_modified_gmt) AS date FROM $wpdb->posts WHERE post_status IN ('publish','inherit') AND post_type IN ('" . implode( "','", get_post_types( [ 'public' => true ] ) ) . "') GROUP BY post_type ORDER BY post_modified_gmt DESC";
	...
	function build_post_type_map( $post_type ) {
		...
		if ( ( isset( $this->options[ 'post_types-' . $post_type . '-not_in_sitemap' ] ) && $this->options[ 'post_types-' . $post_type . '-not_in_sitemap' ] === true ) || in_array( $post_type, [
				'revision',
				'nav_menu_item',
	...
	function sitemap_url( $url ) {
		...
		if ( isset( $url[ 'images' ] ) && ( is_array( $url[ 'images' ] ) && $url[ 'images' ] !== [ ] ) ) {
	...
	function build_tax_map( $taxonomy ) {
		if ( ( isset( $this->options[ 'taxonomies-' . $taxonomy->name . '-not_in_sitemap' ] ) && $this->options[ 'taxonomies-' . $taxonomy->name . '-not_in_sitemap' ] === true ) || in_array( $taxonomy, [
				'link_category',
				'nav_menu',
				'post_format',
	...
	function build_user_map() {
		...
		// Ascending sort.
		usort( $users, [ $this, 'user_map_sorter' ] );
